...
class ViewThumb implements IClassProperties {
  ...
  static moveDoubleTypeX(e: MouseEvent) {
    ...
    const firstThumb = target.querySelector('.range-slider__thumb_first') as HTMLElement;
    const secondThumb = target.querySelector('.range-slider__thumb_second') as HTMLElement;
  ...
  moveDoubleTypeY(e: MouseEvent): void {
    const target = e.target as HTMLElement;
    const track = target.closest('.range-slider') as HTMLElement;
    const inner = target.querySelector('.range-slider__inner') as HTMLElement;
    const targetThumb = target.closest('.range-slider__thumb') as HTMLElement;
    if (!targetThumb) return;
