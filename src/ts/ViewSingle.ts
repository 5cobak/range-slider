...
export default class ViewSingle {
  ...
  private setThumbPosOnInit(settings: IsettingsTypes) {
    ...
    let from = this.settings.from ? this.settings.from : this.settings.min;
    from = +(trackSize / (generalVal / from)).toFixed(1);

    if (settings.from < settings.step || settings.from % settings.step) {
      throw Error('from must be biggest then step and must be aliquot of step');
    }
